...
import { StockMovement, RawMaterial } from '../../types/rawMaterial';
import rawMaterialService from '../../services/rawMaterialService';
import Button from '../ui/button/Button';
import { parseApiError } from '../../services/rawMaterialService';
...
const parseItemName = (itemName: string | object | null | undefined): string => {
  ...
  if (typeof itemName === 'object') {
    ...
    return obj.default || obj["1"] || obj["2"] || obj["3"] || Object.values(obj)[0] || 'Raw Material';
  }
  ...
  if (typeof itemName === 'string') {
    try {
      ...
      if (typeof parsed === 'object' && parsed !== null) {
        return parsed.default || parsed["1"] || parsed["2"] || parsed["3"] || Object.values(parsed)[0] || 'Raw Material';
      }
      return itemName;
    } catch {
      return itemName || 'Raw Material';
    }
  }
...
const StockMovementLog: React.FC<StockMovementLogProps> = ({ rawMaterial, isOpen, onClose }) => {
  ...
  const fetchStockMovements = async (page: number = 1) => {
    try {
      ...
      const response = await rawMaterialService.getStockMovements(rawMaterial.raw_material_id, page);
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              {parseItemName(rawMaterial.item_name)} - Current Stock: {formatQuantity(rawMaterial.quantity.toString())}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-auto p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <Button onClick={() => fetchStockMovements(pagination.current_page)}>
                Try Again
              </Button>
            </div>
            ...
            <div className="text-center py-12">
              ...
              <div className="text-gray-600 dark:text-gray-400">This raw material has no recorded stock movements yet.</div>
            </div>
          ) : (
            <div>
              {/* Timeline */}
