...
const DeleteConfirmationModal: React.FC<DeleteConfirmationModalProps> = ({
  ...
  const itemName = parseItemName(stockItem.item_name, 'Unknown Item');
  const unitName = stockItem.unit ? getDisplayValue(stockItem.unit.name) : '';
  const unitShortName = stockItem.unit ? getDisplayValue(stockItem.unit.short_name) : '';
          ...
          <div className="custom-scrollbar h-auto overflow-y-auto px-2 pb-3">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg mb-6">
              <div className="flex items-center gap-4">
                {stockItem.profile_image_url ? (
                  <img
                    src={stockItem.profile_image_url}
                    alt={`${itemName} image`}
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    HSN: {stockItem.hsn_code}
                  </p>
                  <p className="text-sm text-gray-500 dark:text-gray-500 mt-1">
                    Stock: {parseFloat(stockItem.quantity).toFixed(2)} {unitShortName}
                  </p>
                  {stockItem.barcode_number && (
                    <p className="text-sm text-gray-500 dark:text-gray-500 mt-1 font-mono">
                      Barcode: {stockItem.barcode_number}
                    </p>
                  )}
                  {unitName && (
                    <p className="text-sm text-gray-500 dark:text-gray-500 mt-1">
                      Unit: {unitName} ({stockItem.unit?.unit_type})
                    </p>
                  )}
                  <div className="flex gap-2 mt-2">
                    <p className="text-sm text-orange-600 dark:text-orange-400">
                      Production: ₹{parseFloat(stockItem.production_cost).toFixed(2)}
                    </p>
                    <p className="text-sm text-green-600 dark:text-green-400">
                      Selling: ₹{parseFloat(stockItem.selling_cost).toFixed(2)}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-4 rounded-lg mb-6">
              ...
              <p className="text-red-700 dark:text-red-300 text-sm">
                All data associated with this stock item, including stock levels, pricing information, and movement history, will be permanently deleted.
              </p>
            </div>
