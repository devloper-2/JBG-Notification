...
import { StockItemCreateRequest, StockItemFormData, Unit } from "../../types/stockItem";
...
interface AddStockItemModalProps {
  isOpen: boolean;
  onSave: (data: StockItemCreateRequest) => void;
  onCancel: () => void;
  isSaving: boolean;
...
const AddStockItemModal: React.FC<AddStockItemModalProps> = ({
  ...
  const fetchUnits = async () => {
    try {
      ...
      const response = await stockItemService.getUnits();
  ...
  const validateForm = (): boolean => {
    ...
    } else if (!validateBarcodeNumber(formData.barcode_number.trim())) {
      newErrors.barcode_number = "Invalid barcode format (minimum 8 digits)";
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const apiData: StockItemCreateRequest = {
      ...
      <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
        <div className="px-2 pr-14">
          ...
          <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
            Create a new stock item with details, pricing, and inventory information.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
            ...
            <div className="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
              ...
              <div>
                ...
                <select
                  ...
                  } ${isSaving || unitsLoading || formData.is_malay ? 'opacity-50 cursor-not-allowed' : ''}`}
                  disabled={isSaving || unitsLoading || formData.is_malay}
                >
          ...
          <div className="flex justify-end space-x-3 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <Button type="submit" disabled={isSaving}>
              {isSaving ? "Creating..." : "Create Stock Item"}
            </Button>
          </div>
        </form>
      </div>
